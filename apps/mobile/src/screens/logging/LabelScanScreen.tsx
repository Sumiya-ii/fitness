...
import { View, Text, ScrollView, Pressable, Image, Alert, Animated } from 'react-native';
...
type Props = LogStackScreenProps<'LabelScan'>;
    ...
    <View className="items-center py-12 px-8">
      <Animated.View style={{ transform: [{ scale: pulse }] }}>
        <View className="h-20 w-20 rounded-full bg-surface-secondary items-center justify-center mb-5">
          <Ionicons name="document-text" size={40} color="#1f2028" />
        </View>
      </Animated.View>
      <Text className="text-text font-sans-semibold text-lg mb-1">Reading nutrition label...</Text>
...
export function LabelScanScreen() {
  ...
  const pollDraft = useCallback(async (draftId: string, attempt = 0) => {
    ...
    try {
      ...
      if (d.status === 'failed') {
        setError('Label analysis failed. Please try again.');
        setAnalyzing(false);
        return;
      }
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      const note = `Label: ${baseItems.map((i) => i.name).join(', ')}`;
    ...
    <View className="flex-1 bg-surface-app">
      <SafeAreaView edges={['top']} className="flex-1">
        ...
        <View className="flex-row items-center px-4 py-3 border-b border-surface-border">
          <BackButton />
          <Text className="ml-3 text-lg font-sans-semibold text-text flex-1">
              ...
              {hasItems ? (
                ...
                <View className="items-center py-10">
                  ...
                  <Text className="text-text-secondary text-sm text-center mb-5">
                    Try a clearer photo with the nutrition facts fully visible
                  </Text>
