...
- [x] **Install Expo CLI** — Using `npx expo` commands in this repo for prebuild, run, and tunnel workflows.
...
- [x] **Wire Firebase Auth SDK** — Auth screens now use Firebase email/password sign-in and sign-up, then send Firebase ID tokens to API via `Authorization: Bearer <token>`.
- [x] **Configure app.json** — Expo config now includes app name, iOS/Android bundle identifiers, build numbers, and iOS permission usage descriptions for installable native builds.
...
- [x] 2026-03-06: QPay payment flow hardening — Secured callback verification using optional `QPAY_CALLBACK_TOKEN`, added amount/currency validation before activation, made invoice finalization idempotent with transactional `pending -> paid` transition, added pending invoice TTL handling (`QPAY_INVOICE_TTL_MINUTES`, default 30) with `expired` status, improved Subscription screen with manual status refresh + timeout/error states, refreshed subscription badge on screen focus, and added QPay service tests for security/expiry/idempotency paths.
...
- [x] 2026-03-06: Auth/onboarding UI consistency pass — standardized Welcome, SignIn, SignUp, and onboarding step layouts to a dark gradient, high-contrast card-based style aligned with the main CalAI-inspired screens, and applied global dark navigation theme + light status bar in the app shell.
- [x] 2026-03-06: Mobile design token single source of truth — added centralized `src/theme/tokens.json` + `src/theme/index.ts`, wired Tailwind + React Navigation to those tokens, and updated shared UI/navigation components to consume semantic theme colors instead of local hardcoded hex values.
- [x] 2026-03-06: Light neutral brand refresh — updated mobile design tokens to a light monochrome palette, restyled shared buttons to pill-style with gray disabled state, switched app status bar/navigation surfaces to light mode, and refactored onboarding screens to align with the provided clean gray visual direction.
- [x] 2026-03-06: Feature documentation system — added `docs/features/` living docs with one replaceable document per main feature, including current logic, data model references, API surface, and known gaps for PM/dev handoff.
- [x] 2026-03-06: Agent guardrail for continuous feature docs sync — updated `AGENTS.md` so any feature logic/data/API/UX change must also refresh matching `docs/features/` living docs in-place (no historical append format).
- [x] 2026-03-06: Mobile-wide light theme consistency pass — replaced remaining dark screen surfaces/text classes and key hardcoded colors across home/log/progress/settings/auth/logging/weekly summary/telegram/subscription flows so the overall background/text treatment now follows the provided light reference direction.
- [x] 2026-03-06: Button contrast hardening pass — enforced inverse text for primary/danger shared buttons, kept loading states readable by not applying disabled color overrides while loading, and fixed selected primary custom button labels (home actions, progress tabs, quick-add chips, profile activity chips) to avoid low-contrast text.
- [x] 2026-03-06: Weekly progress calendar on Home — added swipeable week pagination (Sun-Sat), tappable day selection that refetches dashboard metrics/meals for that date, and per-day circular calorie indicators (dashed empty, partial progress, full goal) aligned with the light home reference.
- [x] 2026-03-06: Profile BMI derivation — added backend BMI calculation utility and surfaced `bmi` on `/profile` responses using latest `WeightLog` weight with `Profile.weightKg` fallback.
- [x] 2026-03-06: Onboarding/auth routing hardening — unauthenticated flow now starts at Welcome (Sign In/Sign Up entry), authenticated flow now checks backend `GET /onboarding/status` as source of truth before routing to Setup vs Main, and Welcome now includes an optional "See how it works" path to the intro carousel.
- [x] 2026-03-06: UI primitive standardization pass — introduced reusable `IconButton` and `AuthProviderButton` components, exported them in the shared UI index, and refactored Welcome/SignIn/SignUp to use shared button primitives and consistent link/icon contrast styles instead of duplicated one-off button patterns.
- [x] 2026-03-07: Phase 1 product definition alignment — added `docs/phase-1-product-definition.md` with concrete v1 decisions for promise, target user, transformation, daily habit, weekly outcome, churn risks, scope boundaries, business model, product posture, and primary success metric.
- [x] 2026-03-15: Mobile UI ergonomics pass — fixed top clipping/tap-target issues by increasing shared icon/button hit areas, making Log Home recents cards responsive to viewport width, removing nested/dead pressables in recent cards, and adding keyboard-safe sign-in/sign-up form containers.
- [x] 2026-03-15: Design audit implementation pass — reduced hardcoded color usage on core screens, localized key auth/home/progress/onboarding copy, added accessibility labels for icon-only controls, and replaced static `Dimensions` usage with `useWindowDimensions` for responsive layout behavior.
